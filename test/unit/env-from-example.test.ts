...
describe('detectType', () => {
  ...
  it('detects HTTP URLs', () => {
    expect(detectType('http://localhost:3000', 'API_URL')).toBe(
      'network/url',
    );
  });
...
describe('coerceToType', () => {
  ...
  it('leaves string types as-is (trimmed)', () => {
    ...
    expect(coerceToType('https://x.com', 'network/url')).toBe(
      'https://x.com',
    );
  });
